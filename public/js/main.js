...
$form.addEventListener("submit", async (e) => {
  ...
  // get location
  const location = $form.children[0].children[0].value.trim();

  // empty value
  if (!location) return alert("You have to provide a location first");

  // making request to the server to get the forecast and the map to display on the screen
  const data = await (await fetch(`/weather?location=${location}`)).json();

  // if there is an error stop the execution and tell the user about the proplem
  if (data.error) {
